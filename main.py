...
def message_passing_fn(config: Dict[str, Any]) -> None:
    ...
    mp = NAME_TO_MP[config["mode"]]()
    h = node_feat
    for i in range(config["max_depth"]):
        cur_save_path = os.path.join(config["save_dir"], "{}_{}.npy".format(config["mode"], str(i + 1)))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mode", choices=["message_passing", "train", "eval"], required=True)
